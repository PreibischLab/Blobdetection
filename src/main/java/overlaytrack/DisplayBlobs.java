	...
	public static void Displaydetection (RandomAccessibleInterval<FloatType> img, ArrayList<FramedBlob> frameandblob){
		...
		for (int index = 0; index < frameandblob.size(); ++index){
			...
			if (frame == currentframe){
				...
				AddGaussian.addGaussian(groundframe, frameandblob.get(index).Blobs.Intensity, frameandblob.get(index).Blobs.location, new double[] {frameandblob.get(index).Blobs.maxextent/2,frameandblob.get(index).Blobs.maxextent/2});
